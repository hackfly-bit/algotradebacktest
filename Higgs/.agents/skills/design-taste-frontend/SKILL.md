...
## HIGGS OVERRIDE (dominant — product dashboard)

This repo is **Higgs**, a Django backtesting cockpit. Ignore the landing-page default.

- **Design read:** product dashboard for a solo trader-developer, Linear/research-terminal language, Tailwind CSS v4 + Django templates + HTMX. Not a marketing site.
- **Dials (locked):** `DESIGN_VARIANCE: 5` · `MOTION_INTENSITY: 3` · `VISUAL_DENSITY: 9`
- **Tokens:** read `docs/DESIGN.md` before any HTML. One teal accent. Light + dark. Sans + mono numbers.
- **Allowed motion:** hover, focus-visible, HTMX swap. No GSAP pin/scrub.
- **Real UI:** KPI chips, dense tables, Chart.js, filters, empty states. No fake dashboards made of empty divs.
- **Map "Tailwind SaaS":** utilities + `dark:` only. Do not add Fluent, Carbon, or React/shadcn.

If a later section says "not dashboards", this override wins.

---

> Original skill also covers landing pages. Higgs uses the dashboard override above.
> Every remaining rule is **contextual**. First read the brief, then pull only what fits.
